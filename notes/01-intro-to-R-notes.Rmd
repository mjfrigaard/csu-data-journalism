...
R is an object-oriented, functional programming scripting language, which is the technical way of saying R uses functions (actions) to perform operations objects (data). R is free and open-source software (FOSS) with a massive global community of users and developers who have helped create and maintain tools for data manipulation, graphics, statistics, and machine learning.

Generally speaking, functions are similar to _verbs_, and objects are more like _nouns_.

Functions typically take some object as an `input`s and return an object as an `output`s:

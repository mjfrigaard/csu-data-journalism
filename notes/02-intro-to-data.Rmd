...
I tend to avoid abstract descriptions of what data are (or is) because they remove the necessary elements we need to make sense of them/it (i.e., "*why should we care about these/this data*?"). 
...
In this course, we're going to focus on the connection between the "data" and "measurement", because deciding what gets measured (and how its measured) is a human activity, and in many cases these decisions are just as important as the data itself.

## Data are measurements, and measurements are approximations 

Measurements and metrics are usually intended to capture things that matter (otherwise, why measure them?), but it's also important to remember that **all measurements are approximations**. It's unlikely that the question we're trying to answer has a dataset that *perfectly* captures the phenomena of interest. In statistics, a widespread saying is, "[all models are wrong, but some are useful.](https://en.wikipedia.org/wiki/All_models_are_wrong)" While models use measurements and data to explain or predict something, data visualizations visually display these approximations.  
...
As we can see, even a seemingly simple question requires qualifications and operational definitions we need to describe. Before we can design a data visualization, we need to frame the question in a way that can be answered with the available data (or we need to collect data to answer we're interested in), then we can start determining what the best visualization is for displaying the solution.
...
We're going to be primarily using `tibble`s throughout the course because they have more predictable and reliable behaviors.

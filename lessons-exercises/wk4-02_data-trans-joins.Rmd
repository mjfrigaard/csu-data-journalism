...
Below is a subset of the `endorsements_2020` dataset. Read more about the `endorsements_2020` dataset by typing `??endorsements_2020` into the console. 
...
In another example of un-tidy data, we'll look at the dataset below which comes from 

```{r}
fandango %>% 
  select(film, year, rottentomatoes_user, rt_user_norm) %>% 
  mutate(rottentomatoes_scores = paste0(rottentomatoes_user, " ; ", rt_user_norm)) %>% 
  select(film, year, rottentomatoes_scores)
  separate_rows(fandango_scores, sep = " ; ")
```

`rottentomatoes_user` and `rt_user_norm`

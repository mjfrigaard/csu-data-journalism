...
`tidyr` is part of the core `tidyverse` packages, so we install and load this meta-package below. We're also going to be using data from the [`fivethirtyeight` package](https://fivethirtyeight-r.netlify.app/).
...
Below is a subset of the `endorsements_2020` dataset. 


```{r EndorseSample}
EndorseSample <- tidyr::unite(data = endorsements_2020, "location", c(state, city), sep = ", ", remove = FALSE) %>% 
  filter(!is.na(city) & !is.na(date)) %>% 
  slice_sample(n = 10) %>% 
  select(date, position, location, endorser, endorsee, 
         points, order)
write_csv(x = EndorseSample, "data/EndorseSample.csv")
```

...
R also comes with a few functions for creating dates (`Sys.Date()` and `Sys.time()`). 
...
```{r is.double-vec_date}
is.double(vec_date)
attributes(vec_date)
```

The number for each date is accessible using the `unclass()` function:

```{r unclass-vec_date}
unclass(vec_date)
```

The number are related to the [UNIX Epoch time](https://en.wikipedia.org/wiki/Unix_time), which is January 1, 1970 (00:00:00 UTC). 
...
Date-times contain a bit more information than dates. In R, the `POSIX` date format is the time since January 1, 1970 (in the UTC time zone), measured to the nearest second.  

`POSIXct` represents this as a numeric vector, and `POSIXlt` is list of named date/time vectors.
...
For `vec_datetime_ct`, we can access the epoch time with `unclass()`

```{r unclass-vec_datetime_ct}
unclass(vec_datetime_ct)
```

In `vec_datetime_lt`, each date/time measurement is in a named vector: 

```{r unclass-vec_datetime_lt}
unclass(vec_datetime_lt)
```


For the `"Date"` class, the number represents the number of days, and for the `"POSIX` classes, this represents the number of seconds (to the nearest second) since the epoch. 

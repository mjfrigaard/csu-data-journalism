...
# fs::dir_tree("data")
wikipedia_top_pharm_export_file <- paste0("data/", lubridate::today(), "-TopPharmCompRaw.csv")
readr::write_csv(x = TopPharmCompRaw, file = wikipedia_top_pharm_export_file)
```

```{r import-TopPharmCompRaw, include=FALSE, eval=TRUE}
TopPharmCompRaw <- readr::read_csv("https://raw.githubusercontent.com/mjfrigaard/csuc-data-journalism/main/lessons-exercises/data/2021-10-13-TopPharmCompRaw.csv")
```
...
## `largest_market_cap_us_bil` & `largest_market_cap_date` {.tabset}
...
## (Optional) convert ranking to factor {.tabset}  
...
We now have a dataset we can use for modeling or visualizations! We'll export this to the `data` folder.

```{r export-TopPharmComp}
# fs::dir_tree("data")
wikipedia_top_pharm_export_file <- paste0(lubridate::today(), "-TopPharmComp.csv")
wikipedia_top_pharm_export_file
```

```{r wikipedia_top_pharm_export_file}
readr::write_csv(x = TopPharmComp, file = wikipedia_top_pharm_export_file)
```

...
# convert birthdate to 1900s --------------------------------------------
# create function to adjust century
# assume 1915 as cutoff
adjustCentury <- function(d, threshold = 1915){
  y <- lubridate::year(d) %% 100
  if (y > threshold %% 100) lubridate::year(d) <- 1900 + y
  d
}
# convert dob with adjustCentury -----------------------------------------
# DataSet <- DataSet %>%
#     mutate(new_dob = adjustCentury(lubridate:::mdy(DataSet$old_dob)))
